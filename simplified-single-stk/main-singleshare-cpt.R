# 1. please keep all files folders, since they are different input for the final output
# 2. The graph of the simulation of the stock price is in the the folder named "PDF"
# 3. set off or comment the funcation fn_cpt_stationary.R or HWV may cause problem
# 4. try to make the "main_dir" to be the same of the path of the folder "simplified-single-stk"


# The 1st time ussage, use TRUE
# if alway fundemental files are in the predetermined folder, then one can set the "First_Time = FALSE"
...
# For instance: main_dir = "/home/weizikai/Desktop/gitupdate/stks-simulator/simplified-single-stk/"
# All stock files should be in ./Data
main_dir = paste0(here(),"/")
main_dir = "/home/weizikai/Desktop/gitupdate/stks-simulator/simplified-single-stk/"
...
#date = read.csv(stk_file_name, header=TRUE)

 # paste(main_dir, "Data/", sep = "", collapse = "")
Files = list.files(path = stks_dir)
Len_Files = length(Files)
stk_name = str_sub(Files, 1, 7) # stk_name = str_sub(Files[i])
...
for(NoData in Days ){  
  ...
  if (j3!=j2){
    ...
    #x1=range(as.Date(StockPrice$Date[i:j2],"%d/%m/%Y"))
